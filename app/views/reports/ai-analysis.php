...
<style>
  ...
  *::after {
  ...
  .ai-section-block.alert {
    border-left-color: var(--amber);
    background: #fffbeb;
  }

  .ai-section-block.tip {
    border-left-color: var(--green);
    background: #f0fdf4;
  }

  .ai-section-block.goal {
    border-left-color: var(--brand);
    background: var(--page);
  }

  .ai-section-block.good {
...
<script>
  ...
  async function gerarAnaliseIA() {
    ...
    <p style="text-align:center;font-size:.82rem;color:var(--text-3);padding-bottom:1rem">
      Analisando seus gastos e preparando seu plano semanal…
    </p>`;
    ...
    try {
      ...
      // 1. Gera a análise principal (o que aparece na tela)
      const res = await fetch('/relatorios/analyze', {
        method: 'POST',
        body: fd
      });
      ...
      // 2. GATILHO SILENCIOSO: Gera as 7 dicas para o banco de dados
      // Não precisa de await aqui se não quiser travar a tela pro usuário, 
      // mas vamos colocar para garantir que o processo termine.
      await fetch('/relatorios/gerarDicasSemanais', {
        method: 'POST'
      });

      console.log("Dicas semanais agendadas com sucesso!");
